...
public class WeightedQuickUnionWithPathOptimization {
    private int[] parent;
    private int[] size;

    public WeightedQuickUnionWithPathOptimization(int n) {
        parent = new int[n];
        size = new int[n];
        for (int i = 0; i < n; i++) {
            parent[i] = i;
    ...
    private int findRoot(int p) {
        ...
        while (root != parent[root]) {
            // Path optimization: splitting or halving
            parent[root] = parent[parent[root]]; // Path splitting
    ...
    public static void main(String[] args) {
        WeightedQuickUnionWithPathOptimization uf = new WeightedQuickUnionWithPathOptimization(10); // Create an object
                                                                                                    // for 10 elements
